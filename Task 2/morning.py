...
def filter_data_for_day(data, shift_date):
    ...
    # Filter data for the morning shift (same day login and logout)
    filtered_data = data[
...
def calculate_morning_shift(data):
    ...
    # Only include days where break time is more than 60 minutes
    if total_break_time.total_seconds() / 60 > 60:
        results = {
            'Working Day (Date)': first_login.date(),
